...
export default function InputPage() {
  ...
  try {
    const response = await fetch("http://localhost:5000/predict", {
      ...
      body: JSON.stringify({
        model: region.toLowerCase(),
        latitude: parseFloat(lat),
        longitude: parseFloat(lo),
        temp_c: 30, // placeholder
        rh: 45,
        wind_ms: 3,
        rain_mm: 0,
        NDVI: 0.7,
    ...
    navigate(
      `/results?region=${region}&risk=${data.risk}&value=${data.fire_probability}`
    );
...
return (
  ...
  <select value={region} onChange={(e) => setRegion(e.target.value)}>
    <option value="orissa">Orissa (India)</option>
    <option value="algeria">Algerian Forest</option>
...
export default function InputPage() {
  ...
  const [position, setPosition] = useState(null);
  const [searchQuery, setSearchQuery] = useState('');
  const [region, setRegion] = useState('orissa');
