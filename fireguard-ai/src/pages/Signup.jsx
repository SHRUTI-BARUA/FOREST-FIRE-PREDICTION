...
const Signup = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    const usernameTrim = username.trim();
    const passwordTrim = password.trim();
    const phoneTrim = phoneNumber.trim(); // Trimmed but not validated via Regex
    ...
    try {
      const { data } = await axios.post(
        ...
        { 
          ...
          phoneNumber: phoneTrim // Included in request
        },
        { withCredentials: true }
      );
    ...
    setInputValue({ email: "", password: "", username: "", phoneNumber: "" });
  };
